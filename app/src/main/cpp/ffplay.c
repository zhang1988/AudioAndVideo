...
/* Minimum SDL audio buffer size, in samples. 最小音频缓冲*/
...
typedef struct VideoState {
    ...
    unsigned int audio_buf_size; /* in bytes */         //音频缓冲大小
    ...
    ///显示类型 ,分别是：无显示，显示视频，显示波浪（音频），自适应过滤器
    enum ShowMode {
    ...
    char filename[1024];                                //
    int width, height, xleft, ytop;                     //宽高，左上角坐标
    int step;                                           //步进
